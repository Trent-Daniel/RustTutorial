...
fn read_and_combine(f1: &str, f2: &str) -> Result<
String, io::Error> {
    let mut s1 = match fs::read_to_string(f1) {
        Ok(s) => s,
        Err(e) => return Err(e)
    };
    // The above match and the below line are equivalent. The ? operator can only be used on
    // functions that return a Result enum
